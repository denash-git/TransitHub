...
По умолчанию включён опциональный hack:

- `FORCE_DARK_THEME=true`

Он не меняет `x-ui.db`.
Вместо этого nginx подставляет короткий JS, который записывает в браузерный `localStorage`:

- `dark-mode=true`

Это сделано потому, что upstream `3x-ui` хранит тему в браузере, а не в серверных настройках.

Если нужно отключить этот hack:

```env
FORCE_DARK_THEME=false
```

После изменения достаточно перерендерить конфиги и пересоздать стек.
...
- `CONFIG_PASSWORD`
  пароль панели
- `FORCE_DARK_THEME`
  включает nginx-injection для dark theme
...
- Тёмная тема панели включается через frontend-hack, а не официальной серверной настройкой upstream `3x-ui`.
